...
- 그 밖의 에셋 규칙입니다. 일러스트·아이콘은 사진과 같은 파이프라인이나 SVG로 갑니다. 배경음악은 한 곡 3~5MB 이내로 두고 탭해서 재생합니다. 영상은 YouTube 임베드나 5MB 이하 클립에 클릭 재생으로 갑니다. 20MB 영상을 5천 회 재생하면 100GB로 Hobby 전송 한도를 다 쓰기 때문입니다. 파일 하나가 50MB를 넘지 않게 합니다(GitHub 경고선). 어떤 에셋이 있는지는 U-4입니다.
...
| 배포 | 하루 수 회 | 100회/일 | 큐잉 |
| 배경음악 (있으면) | 5천 회 × 약 5MB ≈ 25GB/월 | 전송 100GB/월에 합산 | 사진과 합쳐 40GB 안팎 |
| 영상 (있으면) | 임베드면 0. 자체 호스팅 5MB 클립이면 ≈ 25GB/월 | 전송 100GB/월에 합산 | 20MB 자체 호스팅은 한도 소진 |

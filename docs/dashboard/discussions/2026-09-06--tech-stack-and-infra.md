...
| 5 | 이미지 변형은 사전 생성합니다. 원본은 git 밖에 두고, 스크립트가 만든 폭 3~4단계 webp·블러 placeholder·매니페스트만 `public/`에 커밋하며, next/image는 custom loader로 그 파일을 가리킵니다. Vercel 런타임 이미지 최적화는 쓰지 않습니다 | Claude 제안 | 잠정 (이산하 확인 대기) |
...
| 9 | 배포 경로는 GitHub push → Vercel Git 연동입니다. `main` → production, 그 밖의 브랜치 → preview. 부트스트랩 결정 7과 짝입니다 | Claude 제안 | 잠정 (이산하 확인 대기) |

...
- 이 레포의 브랜치는 `main`(배포 기준)과 `develop`(통합, 2026-09-06 생성)입니다. story는 develop에서 분기합니다. 배포 매핑(2026-09-06, Vercel Git 연동): `main` push → production https://wedding-invitation-web-mu.vercel.app/, 그 밖의 브랜치 push → preview URL. develop→main 머지가 곧 배포입니다

## git 규약

- 원격 = `origin` → `git@github.com-lazystar:sana-lazystar/wedding-invitation-web.git`. GitHub 계정 `sana-lazystar` 전용 SSH 별칭(`~/.ssh/config`의 `github.com-lazystar`)을 씁니다. 기본 브랜치는 `main`, 통합 브랜치는 `develop`. 호스팅은 Vercel Hobby(프로젝트 `wedding-invitation-web`, 2026-09-06 연동. 정본은 `docs/ontology/topology/system-context.md`)

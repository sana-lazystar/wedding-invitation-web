...
Scene4는 사진 종이(`.photo-paper`. 흰 테두리 4px에 사진을 붙인 것, 폭의 3/4에 오른쪽 정렬, -0.7° 기울임. T56)가 위, 토끼 메모지(`.note--memo`. 쪽지와 같은 부품, 왼쪽 위에 토끼 1)가 아래입니다. 메모지는 글 크기만큼입니다(T61. `width: fit-content` · 여백 20/16. 사진 종이 뒤로 26px 들어가 겹치므로 위만 44px이고 첫 줄은 사진 아래 18px에서 시작합니다). 어릴 적 사진 컷아웃(헬리콥터, `.note__stamp`, 높이 160px)은 글 아래 칸(`.note__row`)의 고정 크기 셀(`.note__stamp-cell`, 164×96)에 절대 위치로 붙어 셀 밖으로 오른쪽 34px · 아래 64px 삐져나옵니다(T87 · T88). 글은 이산하가 정한 줄바꿈(`<br>`, 넉 줄) 그대로 전체 폭이고 셀이 그 아래라 겹침이 없습니다. 메모지 높이는 글 + 셀 + 여백이고 컷아웃은 종이 아래로 44px 걸칩니다. T87에서는 마지막 문장이 셀 옆 열에서 흘렀는데(네모 영역, T62), 정한 줄바꿈이 폭을 다 써서 옆 열이 없어져 셀을 아래로 내렸습니다(T88). 전에는 본문 안의 float라 메모지가 컷아웃 높이까지 커졌고(T61 · T62 · T67), 메모지가 flex 항목이라 float를 늘 품으므로 float로는 걸치게 할 수 없었습니다. 글과 컷아웃 사이는 8px입니다(16의 반, T87). 신부(Scene5)는 셀이 왼쪽(`.note__row--left`, 75×122)이고 컷아웃(216px)이 왼쪽 12px · 아래 94px 삐져나옵니다(종이 아래 74px. 셀 + 삐짐 = 216이라 컷아웃 윗변이 칸 윗변에 맞아 여섯 줄의 마지막 줄을 덮지 않습니다, T88). 본문 24px 들여쓰기(T60)는 컷아웃이 글 옆에 없어져 뺐습니다.

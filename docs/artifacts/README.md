...
편지봉투입니다(디자인 논의 T35 · T36 · T37). 봉투가 화면 높이만큼 확대돼 있다가(좌우가 잘림) 물러나 전체가 보이고(폭 80%, 가운데), 뚜껑이 봉인을 단 채 위로 180° 젖혀지고, 커버(Scene1)가 카드로 올라오는 동안 봉투는 아래로 내려가고, 카드가 화면 가운데를 기준으로 커져 화면을 채웁니다. 5.4초쯤이고 오른쪽 아래 "넘어가기"나 화면 어디를 탭해도 건너뜁니다. 시작할 때 맨 위로 스크롤합니다(새로고침해도). 움직임 줄이기 설정이거나 앵커로 들어오면 재생하지 않습니다. 매번 재생합니다.

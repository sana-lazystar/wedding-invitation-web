...
공유하기는 Next.js에서 카카오톡 공유(JS SDK 2.8.3, 지도와 같은 앱의 JavaScript 키)입니다. 메뉴를 열 때 SDK를 미리 싣고, 누르면 피드 템플릿(제목 · 설명 · `/og/share.jpg` · 단추 "청첩장 보기" · "위치 보기")으로 보냅니다. 키가 없거나 SDK를 못 실으면 기기 공유 창, 그것도 없으면 주소 복사 + 알림입니다. 조립본은 기기 공유 창 · 주소 복사만 합니다. OG 미리보기는 `src/app/layout.tsx`의 `metadata`(metadataBase · openGraph · twitter)이고 그림은 쓰임에 따라 둘입니다(T147). 주소를 붙여넣었을 때의 미리보기는 `public/og/share.jpg`(1600×800, 2:1. 카카오가 800×400으로 자르므로 미리 2:1로 잘라 둡니다), 카카오톡 공유 메시지는 `public/og/share-tall.jpeg`(2595×3460, 3:4. 이산하가 T148에 자기 사진으로 갈았고 그때 확장자가 `.jpeg`가 됐습니다. 세로로 크게 뜨고, 카카오가 얼굴 기준으로 다시 자르지 않도록 두 사람을 가운데 두고 미리 잘라 둡니다)입니다. 원본은 `../design/og/og-image-v2.jpeg`이고 처음 것은 `og-image.jpeg`였습니다(T132 · T146).

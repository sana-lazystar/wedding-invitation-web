...
- Scene11 오시는 길(12쪽) · Scene12 하객 안내(13쪽) · Scene13 마음 전하는 곳(14쪽)은 T110에 Claude가 구성하고 T111에 테마에 맞게 다시 짰습니다. 크림색 바탕 위 흰 종이 한 장씩(`.sheet`. 쪽지 결 · 위 가운데 테이프 · 올리브 가지 + 고운바탕 제목)이고, 마음 전하는 곳에는 봉투의 장미 봉인이 있습니다. 계좌 행은 누르면 번호가 복사됩니다. 아래 "오시는 길 · 하객 안내 · 마음 전하는 곳"
...
Scene11~13(와이어프레임 12~14쪽, 디자인 논의 T110 · T111)은 이산하가 "테마를 토대로 자연스럽게 구성하라"고 맡긴 것입니다. T110에 흰 띠 + 굵은 노토 산스 제목으로 짰다가 정보 페이지처럼 보여 T111에 다시 했습니다. 안내 장면 셋은 크림색 바탕 위 흰 종이 한 장씩(`.sheet`)입니다. 쪽지와 같은 종이 결(`note.png`), 기울임 없음, 위 가운데 마스킹테이프(`.sheet__tape`. 사진 뷰어 인화지와 같은 `tape-short.png`, -4°), 머리(`.sheet__head`)에 올리브 가지(`.sheet__branch`. Scene2 핵심 정보와 같은 `branch.png`, 28px)와 고운바탕 제목(`.sheet__title`. 18px · 700 · 자간 0.12em)입니다. 작은 표제(`.sheet__label`. 주차 · 지하철 · 버스 · 신랑 측 · 신부 측)는 고운바탕 15px에 앞의 오크색 점(떠 있는 메뉴의 점과 같은 것)이고, 본문(`.sheet__text` · `.route__text`)은 노토 산스 15px입니다. 캐릭터는 없습니다(격식 구간). 사진첩의 흰 띠 · 굵은 왼쪽 제목(`.plain`)은 이산하 지시(T108)라 그대로입니다.

오시는 길(`#directions`)은 식장 이름(고운바탕) · 주소(`.venue`. Scene2와 같은 값) 가운데, 약도(`.map-paper`. 흰 테두리 4px에 -0.6° 기울인 사진 종이. 안은 크림색 자리표시. 이산하가 그림을 주면 `<img>`로 교체합니다), 알약 셋(`.chip`. 가운데 정렬, 오크색 테두리. 네이버 지도 · 카카오맵은 식장 이름 검색 링크라 좌표가 없어도 되고, 주소 복사는 아래 복사 알림을 씁니다), 주차 · 지하철 · 버스(`.route`. 주차가 맨 앞)입니다. 노선 이름은 색 알약(`.line`. 7호선 올리브 · 수인분당선 노랑 · 간선 파랑 · 지선 초록)입니다. 하객 안내(`#guide`)는 종이 한 장에 가운데 정렬 격식체 두 문단과 사이 가는 줄(`.sheet__rule`)입니다. 신부대기실 시각은 추정(U-5)입니다. 마음 전하는 곳(`#gift`)은 종이 오른쪽 위에 봉투의 장미 봉인(`.gift__seal`. `public/intro/rose-seal.png` 44px, 8°. 마음을 봉투에 담는다는 뜻), 안내 문장, 신랑 측 · 신부 측 각 세 행입니다. 행 전체가 단추(`.account`. 크림색 종이 조각, 관계 · 이름 위, 은행 · 번호 아래, 오른쪽에 오크색 복사 표시)이고 누르면 계좌번호만(은행명 없이) 복사되며 화면 아래 가운데 알림(`.toast`)이 1.6초 뜹니다. 접지 않고 늘 펼칩니다. 성함 · 번호는 자리표시(디자인 결정 6)입니다. 복사는 clipboard API, 없으면(http · 옛 브라우저) 숨긴 textarea와 execCommand입니다. 떠 있는 메뉴의 "연락처"(`#contact`)는 와이어프레임에 블록이 없어 아직 갈 곳이 없습니다.
...
| 에셋 | 새것 없음. 종이 `public/paper/note.png` · 테이프 `tape-short.png` · 가지 `public/scene2/branch.png` · 봉인 `public/intro/rose-seal.png`을 다시 씁니다 |
| Next.js | `src/app/page.tsx`의 같은 구획 · `ACCOUNTS` · `VENUE_ADDRESS` · `copyText` · `showToast` · `CopyIcon` |
| 기다리는 것 | 약도 그림(Q11) · 실제 계좌와 성함 · 신부대기실 시각(U-5) · 연락처 블록 여부 |

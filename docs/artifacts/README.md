...
오른쪽 아래 떠 있는 메뉴(`.fab`, 디자인 논의 T132. 전에는 오크색 동그라미에 ≡ 아이콘이라 흔한 앱 단추 같았습니다)는 테이프로 붙인 작은 쪽지(`.fab__button`, 64×46 · 2°) 위 손글씨 "메뉴"이고 열리면 "닫기"로 바뀝니다. 누르면 그 위로 쪽지 한 장(`.fab__menu`, 168px · -1.5° · 위 가운데 테이프)이 8px 내려앉으며 나타나고 네 항목(`.fab__item`. 오시는 길 `#directions` · 사진첩 `#gallery` · 마음 전하는 곳 `#gift` · 공유하기 단추)이 오크색 점과 손글씨 18px로 있습니다. 종이 결 · 테이프 · 그늘 · 기울기는 이야기 구간 쪽지의 부품 그대로입니다. 바깥을 누르거나 Esc, 항목을 고르면 닫힙니다.

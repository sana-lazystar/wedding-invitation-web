...
- 완료: 진입 장면(편지봉투) · Scene1 커버(와이어프레임 2쪽) · Scene2 핵심 정보(3쪽, 올리브 가지) · 떠 있는 바로 가기 메뉴(T132에 테이프 붙인 쪽지로. 오시는 길 · 사진첩 · 마음 전하는 곳 · 공유하기) · 카카오톡 공유 · OG 미리보기(T132). 아래 "떠 있는 메뉴 · 공유 · OG (쪽지)"
...
오른쪽 아래 떠 있는 메뉴(`.fab`, 디자인 논의 T132 · T134. 전에는 오크색 동그라미에 ≡ 아이콘이라 흔한 앱 단추 같았습니다)는 닫힌 상태가 테이프로 붙인 작은 쪽지(`.fab__button`, 64×46 · 2°) 위 손글씨 "메뉴"입니다. 누르면 그 자리에 쪽지 한 장(`.fab__menu`, 172px · -1°. 테이프 없음)이 8px 내려앉으며 나타나고 네 항목(`.fab__item`. 오시는 길 `#directions` · 사진첩 `#gallery` · 마음 전하는 곳 `#gift` · 공유하기 단추)이 고운바탕 15px로 가는 줄 사이에 있으며 맨 아래에 옅은 "닫기" 줄(`.fab__close`)이 같은 장에 있습니다. 열린 동안 작은 쪽지는 숨깁니다(T134. T132에는 열린 쪽지에도 테이프 · 오크색 점 · 손글씨가 있어 난잡했음). 종이 결 · 그늘은 이야기 구간 쪽지의 부품 그대로입니다. 닫기 · 바깥 누름 · Esc · 항목 선택으로 닫히고 닫기면 작은 쪽지가 초점을 받습니다.

...
- Scene11 오시는 길(12쪽) · Scene12 하객 안내(13쪽) · Scene13 마음 전하는 곳(14쪽)은 T110에 Claude가 구성했습니다. 흰 바탕 정보 구획(`.plain`)에 왼쪽 굵은 제목이고, 하객 안내만 크림색 바탕 위 흰 종이입니다. 계좌 행은 누르면 번호가 복사됩니다. 아래 "오시는 길 · 하객 안내 · 마음 전하는 곳"
...
## 오시는 길 · 하객 안내 · 마음 전하는 곳

Scene11~13(와이어프레임 12~14쪽, 디자인 논의 T110)은 이산하가 "테마를 토대로 자연스럽게 구성하라"고 맡긴 것입니다. T107~T108의 방향(일반적인 정보 구획)을 따라 흰 바탕 구획(`.plain`)에 왼쪽 굵은 제목(`.plain__title`, 사진첩과 같음)이고, 종이 · 크림색 · 오크색 같은 테마 요소만 남겼습니다. 흰(사진첩 · 오시는 길) → 크림(하객 안내) → 흰(마음 전하는 곳)으로 번갈아 듭니다.

오시는 길(`#directions`)은 제목 아래 식장 이름 · 주소(`.venue`. Scene2와 같은 값), 약도 자리(`.map-paper`. 크림색 4:3 상자. 이산하가 그림을 주면 `<img>`로 교체합니다), 알약 셋(`.chip`. 네이버 지도 · 카카오맵은 식장 이름 검색 링크라 좌표가 없어도 되고, 주소 복사는 아래 복사 알림을 씁니다), 주차 · 지하철 · 버스(`.route`. 주차가 맨 앞)입니다. 노선 이름은 색 알약(`.line`. 7호선 올리브 · 수인분당선 노랑 · 간선 파랑 · 지선 초록)입니다. 하객 안내(`#guide`)는 크림색 바탕 위 흰 종이 한 장(`.guide__paper`. 쪽지와 같은 결, 기울임 없음)에 가운데 정렬 격식체 두 문단과 사이 가는 줄이고 캐릭터가 없습니다. 신부대기실 시각은 추정(U-5)입니다. 마음 전하는 곳(`#gift`)은 제목 · 안내 문장 · 신랑 측 · 신부 측 각 세 행입니다. 행 전체가 단추(`.account`. 관계 · 이름 위, 은행 · 번호 아래, 오른쪽에 복사 표시)이고 누르면 계좌번호만(은행명 없이) 복사되며 화면 아래 가운데 알림(`.toast`)이 1.6초 뜹니다. 접지 않고 늘 펼칩니다. 성함 · 번호는 자리표시(디자인 결정 6)입니다. 복사는 clipboard API, 없으면(http · 옛 브라우저) 숨긴 textarea와 execCommand입니다. 떠 있는 메뉴의 "연락처"(`#contact`)는 와이어프레임에 블록이 없어 아직 갈 곳이 없습니다.

| 것 | 자리 |
| --- | --- |
| CSS · 마크업 | `index.html`의 "흰 바탕 구획 공통" · "11. 오시는 길" · "12. 하객 안내" · "13. 마음 전하는 곳" · "복사 알림" 구간, `#directions` · `#guide` · `#gift` 구획, `.page` 뒤의 `#toast`, 스크립트의 복사 부분(`[data-copy]`) |

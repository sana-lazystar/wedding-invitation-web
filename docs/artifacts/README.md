...
- Scene14 마지막(15쪽)은 화면 전체 편지지 "고마움을 봉해 보냅니다."가 진입 장면을 거꾸로 돌려 봉투에 담기고 봉인됩니다(T121). 아래 "마지막 (봉투 닫힘)"
...
Scene14(와이어프레임 15쪽, 디자인 논의 T121)는 진입 장면의 역입니다. 고정 높이 구획(`#closing`, `.block--fixed`)에 화면 전체 편지지(`.closing__letter`. 쪽지와 같은 종이 결)가 있고 가운데에 "고마움을 봉해 보냅니다."(고운바탕 22px · 700)와 "2026. 10. 09"(코모런트 17px)가 있습니다. 구획이 60% 보이면 `is-closing`이 붙어 한 번 재생합니다. 0~1.1초 편지지가 봉투 안 크기로 줄며 조금 내려가고(진입의 커지기 역), 0.8~1.1초 내려가 있던 봉투가 스르륵 나타나며(흐려짐 역), 1.1~2.0초 편지지가 주머니로 들어가는 것과 동시에 봉투가 올라오고(역), 2.0~4.0초 뚜껑이 봉인을 단 채 그림자를 드리우며 닫힙니다(옆면 2.89초에 면과 층 교체, 125° 2.7초에 그림자 층 교체). 끝은 닫힌 봉투가 구획 가운데에 선 채이고 확대(포커스인)는 없습니다. 그 뒤로는 다시 열고 닫힙니다(T122). 첫 재생이 끝나면(뚜껑 닫힘 animationend) `is-settled`가 붙어 키프레임을 끄고 끝 상태를 기본값으로 둔 채 transition으로 움직입니다. 구획이 보이는 동안 스크롤을 올리면 `is-open`: 뚜껑이 1.4초에 열리고(옆면 0.62초에 면 · 층 교체) 0.9초 뒤 편지지가 0.7초에 제 높이의 35%(`--c-ty1`)만큼 나옵니다. 내리면 편지지가 먼저 들어가고(0.7초) 0.4초 뒤 뚜껑이 닫힙니다. 봉투는 제자리입니다. 스크롤 방향은 scrollY 차이로 보고, iOS 페이지 끝 튕김(최대를 넘었다 돌아옴)은 최대 근처 값을 무시해 거릅니다. 스크롤 위치에 각도를 묶는 스크러빙은 마지막 구획이라 여유가 없어 쓰지 않았습니다. 층 구조 · 배율 · 치수 계산은 진입 장면과 같고(변수 `--c-z1` · `--c-drop1` · `--c-ty0` · `--c-ty1` · `--c-s0`, 스크립트가 구획 크기에서 계산해 구획에 넣음) 구획 안 절대 위치라 스크롤과 함께 움직입니다. 봉투 · 뚜껑 · 봉인 · 그림자 부품(`.intro__env` · `.intro__flap` · `.intro__cast` 등)은 진입 장면의 클래스와 그림을 그대로 쓰고, 시작 상태(뚜껑 180° · 안쪽면 · 봉인 뒷면 · 봉투 투명 · 내려간 자리)만 `.closing` 아래에서 덮어씁니다. 편지지 아랫부분이 봉투 밑으로 삐져나오는 것은 앞판 층의 바닥(`.closing__floor`, 구획 바탕색)이 가립니다. 움직임 줄이기면 봉투 층을 숨기고 편지지만 보입니다.

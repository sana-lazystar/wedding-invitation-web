...
- Scene10(11쪽)은 추신 + 만화(위 절반, T102)와 사진첩(아래 절반, T103 · T104)입니다. 마스킹테이프로 붙인 추신 쪽지와 네 컷 만화 종이(누르면 90° 돌린 뷰어), 흰 바탕 구획(`.plain`)에 왼쪽 정렬한 굵은 제목 "사진첩"과 3×3 타일(T107 · T108. 나무 틀 · 표지판은 지움. 누르면 Swiper 뷰어. 흰 테두리 인화지 + 위 가운데 테이프 + 사진 아래 손글씨 장수 + 반투명 종이 단추, T105 손질). Scene9 초대(10쪽)는 이산하가 수정할 것이 있어 뒤로 미뤘고 Scene8과 Scene10 사이에 끼웁니다. 아래 "추신·만화 (뷰어)" · "사진첩 (나무 틀 · Swiper 뷰어)"
...
| 에셋 | 종이 `public/paper/note.png` · 가지 `public/scene2/branch.png` · 봉인 `public/intro/rose-seal.png`을 다시 씁니다. 머리 그림 제안 셋(`assets/icon-directions.svg` 핀과 발자국 · `icon-guide.svg` 안내 종 · `icon-gift.svg` 봉투와 하트. 올리브 가지와 같은 도장 실루엣, 200×200. T112)은 이산하가 고르면 `public/scene11~13/`에 1배로 산출해 `.sheet__branch` 자리에 26px로 넣습니다 |
